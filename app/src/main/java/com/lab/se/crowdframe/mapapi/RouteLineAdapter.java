package com.lab.se.crowdframe.mapapi;
...
/**
 * Created by lenovo2013 on 2017/3/23.
 */
...
public class RouteLineAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = layoutInflater.inflate(R.layout.transit_item, null);
        ...
        switch (mtype) {
            case  TRANSIT_ROUTE:
            case WALKING_ROUTE:
                WalkingRouteLine walkingRouteLine = (WalkingRouteLine) routeLines.get(position);
                holder.name.setText( "线路" + (position + 1));
                holder.dis.setText("距离为：" + walkingRouteLine.getDistance() + "米");
                break;
            case BIKING_ROUTE:
